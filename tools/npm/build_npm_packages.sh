#!/bin/sh
# Build the npm packages for one release: a meta package `llmtrim` (bin shim +
# optionalDependencies) and one package per platform carrying the prebuilt binary
# (esbuild/Biome pattern — npm installs only the matching platform package).
#
